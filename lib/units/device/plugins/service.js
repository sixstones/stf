  ...
  .define(function(options, adb, router, push, apk) {
    ...
    function openService() {
      ...
      return callService(util.format(
        ...
        .then(function(conn) {
          ...
          service.reader.on('error', function(error) {
            log.error(error.stack)
          })
          service.writer = new ms.DelimitingStream()
          service.writer.on('error', function(error) {
            log.error(error.stack)
          })
    ...
    function openAgent() {
      ...
      return stopAgent()
        ...
        .then(function(conn) {
          agent.socket = conn
          agent.socket.on('error', function(error) {
            log.error(error.stack)
          })
          agent.writer = new ms.DelimitingStream()
          agent.writer.on('error', function(error) {
            log.error(error.stack)
          })
    ...
    function runAgentCommand(type, cmd) {
      try {
        ...
        log.error('run Agent Command occur error', error.stack)
      }
    }
    ...
    function runServiceCommand(type, cmd) {
      ...
      try {
        ...
        log.error('run Serivce Command occur error', error.stack)
      }
    ...
    plugin.acquireWakeLock = function() {
      return runServiceCommand(
        ...
        .timeout(30000)
        .then(function(data) {
          log.info('acquireWakeLock , data = ' + data)
    ...
    plugin.releaseWakeLock = function() {
      return runServiceCommand(
        ...
        .timeout(30000)
        .then(function(data) {
          log.info('releaseWakeLock , data = ' + data)
          var response = apk.wire.SetWakeLockResponse.decode(data)
          if (!response.success) {
